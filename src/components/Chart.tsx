import { lnDensity } from "../lib/blackScholes";
import { money, moneySigned } from "../lib/format";
import { styles } from "../styles";
import type { PnlModel } from "../types";

interface ChartProps {
  model: PnlModel | Record<string, never>;
}

export function Chart({ model }: ChartProps) {
  const W = 720;
  const H = 380;
  const pad = { top: 44, right: 44, bottom: 56, left: 44 };
  const plotW = W - pad.left - pad.right;
  const plotH = H - pad.top - pad.bottom;
  const { samples, xMin, xMax, maxGain, markers, dots, hasIv, iv, dte, spot } = model as PnlModel;
  ...
  const yToPx = (y: number) => pad.top + ((yTop - y) / (yTop - yBot)) * plotH;

  const line = samples.map(([x, y]) => `${xToPx(x).toFixed(1)},${yToPx(y).toFixed(1)}`).join(" ");
  const zeroY = yToPx(0);

  // Lognormal probability density overlay
  let densityPts: string | null = null;
  if (hasIv && (iv ?? 0) > 0 && (dte ?? 0) > 0 && (spot ?? 0) > 0) {
    const T = (dte as number) / 365;
    const densities = samples.map(([x]) => lnDensity(x, spot as number, iv as number, T));
