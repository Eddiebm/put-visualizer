...
import type { JournalEntry, MarketCondition } from "../types";
...
export function DayReview({ journal, scanStats, capital }: DayReviewProps) {
  ...
  const todayWins = todayClosed.filter(e => (e.realizedPnl as number) >= 0);
  const todayPnl = todayClosed.reduce((s, e) => s + (e.realizedPnl as number), 0);
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            {(() => {
              ...
              const total = closed.reduce((s, e) => s + (e.realizedPnl as number), 0);
              return [
                { label: "Total trades", value: closed.length, color: undefined as string | undefined },
                { label: "Win rate", value: `${Math.round(wins.length / closed.length * 100)}%`, color: undefined as string | undefined },
