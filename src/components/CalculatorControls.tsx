import { money, money2, moneySigned, trimNum } from "../lib/format";
import { stratPnl } from "../lib/pnl";
import { COMPANIES, MODES, SNAPSHOT_DATE } from "../appConstants";
import { styles } from "../styles";
import type { Mode, PnlModel, StrategyParams } from "../types";

// ─── Quote / premium-quote shapes ─────────────────────────────────────────────
// These aren't shared domain types (they're local UI fetch-status state), so
// they live here rather than in ../types.
...
export function Field({ label, value, onChange, prefix, suffix }: FieldProps) {
  return (
...
interface CompanyPickerProps {
  ...
  onFetchPremium: () => void;
  premQuote: PremQuoteState;
}

export function CompanyPicker({ ticker, quote, onSelect, expiration, onExpirationChange, onFetchPremium, premQuote }: CompanyPickerProps) {
  return (
...
export function PremiumStatus({ premQuote }: PremiumStatusProps) {
  ...
  if (s === "live")
    return (
      <span style={styles.pickerStatus}>
        <Dot color="#3aa56b" /> {money2(premQuote.premium as number)} mid · {premQuote.strike}P {premQuote.expiration}
        {premQuote.requestedStrike !== premQuote.strike ? ` (nearest to ${money2(premQuote.requestedStrike as number)})` : ""}
      </span>
    );
  if (s === "nokey")
    return (
...
export function QuoteStatus({ quote }: QuoteStatusProps) {
  ...
  if (quote.status === "loading")
    return (
      <span style={styles.pickerStatus}>
        <Dot color="#b8c2d0" /> snapshot {money2(quote.price as number)} — checking live price…
      </span>
    );
  if (quote.status === "live")
    return (
      <span style={styles.pickerStatus}>
        <Dot color="#3aa56b" /> live {money2(quote.price as number)}
        {quote.date ? ` · ${quote.date}` : ""}
      </span>
    );
  return (
    <span style={styles.pickerStatus}>
      <Dot color="#cf9a3a" /> offline — snapshot {money2(quote.price as number)} (approx, {SNAPSHOT_DATE})
    </span>
  );
}
...
interface SizingHintProps {
  mode: Mode;
  capital: number;
  perContractCash: number;
  maxContracts: number;
  contracts: number;
  dropPct: number;
  model: Partial<PnlModel>;
  p: StrategyParams;
  onApply: (n: number) => void;
}

export function SizingHint({ mode, capital, perContractCash, maxContracts, contracts, dropPct, model, p, onApply }: SizingHintProps) {
  ...
  const lossAtSize = (n: number): number => {
    const pp = { ...p, shares: n * 100 };
    if (mode === "strangle") return stratPnl((p.callStrike ?? 0) * (1 + dropPct / 100), pp);
    if (mode === "spread") return stratPnl((p.longStrike ?? 0) * 0.97, pp);
