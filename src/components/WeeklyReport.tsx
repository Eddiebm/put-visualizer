...
import { today, weekStartIso, weekLabel } from "../lib/dates";
import { summarizeWeek } from "../lib/journal";
...
export function WeeklyReport({ journal }: WeeklyReportProps) {
  ...
  for (const e of closed) {
    const wk = weekStartIso(e.closedAt as string);
    (weeks[wk] ??= []).push(e);
  }
        ...
        {current ? (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12 }}>
              ...
              <Stat label={`Wins (${current.wins.length})`} value={moneySigned(current.wins.reduce((s, e) => s + (e.realizedPnl as number), 0))} tone="good" />
              <Stat label={`Losses (${current.losses.length})`} value={moneySigned(current.losses.reduce((s, e) => s + (e.realizedPnl as number), 0))} tone="bad" />
