...
interface InsightCardProps {
  ...
  detail?: string;
  note?: string;
  tag?: "rich" | "cheap" | "fair" | string;
}

export function InsightCard({ icon, label, text, detail, note, tag }: InsightCardProps) {
...
export function Scenarios({ cards }: ScenariosProps) {
  return (
      ...
      {cards.map((c) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ ...styles.scenarioPnl, color: isLoss ? "#e14c4c" : "#3aa56b", fontSize: isWorst ? 26 : undefined }}>{moneySigned(c.pnl ?? 0)}</div>
