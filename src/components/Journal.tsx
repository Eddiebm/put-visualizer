...
export function Journal({ journal, onLog, onClose, onDelete }: JournalProps) {
  const closed = journal.filter((e) => e.status === "closed");
  const wins = closed.filter((e) => (e.realizedPnl as number) >= 0);
  const losses = closed.filter((e) => (e.realizedPnl as number) < 0);
  const realized = closed.reduce((s, e) => s + (e.realizedPnl as number), 0);
  const winSum = wins.reduce((s, e) => s + (e.realizedPnl as number), 0);
  const lossSum = losses.reduce((s, e) => s + (e.realizedPnl as number), 0); // negative
  const worst = losses.reduce((m, e) => Math.min(m, e.realizedPnl as number), 0);
...
export function JournalRow({ e, onClose, onDelete }: JournalRowProps) {
  const [px, setPx] = useState("");
  const [currentPrice, setCurrentPrice] = useState<number | null>(null);
  const [currentOptionPrice, setCurrentOptionPrice] = useState<number | null>(null);
  const open = e.status === "open";
  const loss = e.status === "closed" && (e.realizedPnl as number) < 0;
  ...
  function monitorStatus(): MonitorStatus | null {
    ...
    <div style={styles.journalRow}>
      <div style={{ minWidth: 0, flex: "1 1 200px" }}>
        ...
        <div style={styles.journalMeta}>
          ...
          {e.status === "closed" && ` · closed ${e.closedAt} @ ${money2(e.closePrice as number)}`}
        </div>
        {stopLossHit && (
          <div style={{
      ...
      {open ? (
        ...
        <div style={styles.journalActions}>
          <span style={{ ...styles.journalPnl, color: loss ? "#e14c4c" : "#3aa56b" }}>
            {moneySigned(e.realizedPnl as number)}
          </span>
          <button type="button" onClick={() => onDelete(e.id)} style={styles.deleteBtn} aria-label="delete">
            ✕
          </button>
