...
describe("AssignmentView", () => {
  it("renders nothing without a real position", () => {
    const { container } = render(<AssignmentView mode="put" putStrike={0} putPrem={0} contracts={0} spot={0} />);
    expect(container).toBeEmptyDOMElement();
  });

  it("shows shares owned, total cost, and cost basis for a cash-secured put", () => {
    const { container } = render(<AssignmentView mode="put" ticker="AAPL" putStrike={280} putPrem={3.5} contracts={1} spot={284} />);
  ...
  it("mentions the long put lets you exercise instead of holding, for a spread", () => {
    render(<AssignmentView mode="spread" ticker="MSFT" putStrike={370} putPrem={4} longStrike={360} contracts={1} spot={373} />);
  ...
  it("warns that a naked strangle's assignment isn't covered by anything already owned", () => {
    render(<AssignmentView mode="strangle" ticker="TSLA" putStrike={350} putPrem={6} contracts={1} spot={380} />);
  ...
  it("explains a covered strangle's put assignment doubles the share count", () => {
    render(<AssignmentView mode="covered" ticker="AAPL" putStrike={280} putPrem={3.5} contracts={1} spot={284} />);
  ...
  it("omits the position-value comparison when there's no live spot price", () => {
    render(<AssignmentView mode="put" ticker="AAPL" putStrike={280} putPrem={3.5} contracts={1} spot={0} />);
    expect(screen.queryByText(/Position value at/)).not.toBeInTheDocument();
  });

  it("quotes the put's own premium, not a multi-leg credit, in the cost-basis sentence (regression)", () => {
    // For a strangle the total structure credit (put + call, $1,100 here)
    // is a different number from what the cost-basis math actually
    // subtracts (only the put's own $600) — the sentence used to quote the
    // former next to a number computed from the latter, contradicting
    // itself. It must quote whatever it actually subtracted: $600.
    const { container } = render(<AssignmentView mode="strangle" ticker="TSLA" putStrike={350} putPrem={6} contracts={1} spot={380} />);
    expect(container.textContent).toContain("$600");
    expect(container.textContent).not.toContain("$1,100");
    expect(container.textContent).toContain("$344.00/share"); // 350 - 6
  });
});
