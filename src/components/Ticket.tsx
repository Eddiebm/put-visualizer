...
import { money, money2, formatExp } from "../lib/format";
...
export function Ticket({ mode, ticker, expiration, putStrike, putPrem, longStrike, longPrem, callStrike, callPrem, contracts, collateral, tastyConnected, onPlaceOrder }: TicketProps) {
  ...
  const legs =
    mode === "spread"
      ? [
          ...
          `Buy to Open  · ${qty} ${sym} ${exp} ${money2(longStrike ?? 0)} Put · Limit ${money2(longPrem ?? 0)} · Day`,
        ]
      : [`Sell to Open · ${qty} ${sym} ${exp} ${money2(putStrike)} Put · Limit ${money2(putPrem)} · Day`];
  if (mode === "strangle" || mode === "covered")
    legs.push(`Sell to Open · ${qty} ${sym} ${exp} ${money2(callStrike ?? 0)} Call · Limit ${money2(callPrem ?? 0)} · Day`);

  const tag =
    mode === "put"
      ? "cash-secured put"
      : mode === "spread"
      ? `bull put spread — max loss capped at ${money2(putStrike - (longStrike ?? 0) - (putPrem - (longPrem ?? 0)))} per share`
      : mode === "strangle"
