...
interface CoveredPop {
  keepPremium: number;
  assignmentRisk: number;
}

export function popFromDelta(strategy: Mode, legs: Legs): number | CoveredPop | null {
...
export function expectedMove(spot: number, atmIV: number, dte: number): number {
...
export function cushionSigma(spot: number, strike: number, expMove: number): number {
  if (!(expMove > 0)) return 0;
  return (spot - strike) / expMove;
}

// Plain English — what the win-rate number means for a regular person.
export function popPlain(pop: number | null | undefined): string | null {
...
export function cushionPlain(sigma: number | null | undefined): string | null {
