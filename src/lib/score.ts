...
import type { Richness, Grade, Check, MarketCondition } from "../types";
...
export function opportunityScore({ richness, pop, cushion, canAfford, capitalPct, annYield, maxLossPct, hasEarnings }: OpportunityScoreInput): number {
...
interface AutopilotChecksInput {
  richness: Richness | null;
  pop: number;
  canAfford: boolean;
  maxLossPct: number;
...
export function autopilotChecks({ richness, pop, canAfford, maxLossPct, cushion, capitalPct, hasEarnings, earningsDate }: AutopilotChecksInput): Check[] {
  return [
    {
      key: "conditions",
...
export function marketCondition(richnessTags: Array<Richness["tag"]>): MarketCondition | null {
