...
import { entryCollateral, entryRiskNote, entryBadWeekPnl, summarizeWeek } from "./journal.js";
...
describe("entryBadWeekPnl", () => {
  ...
  it("computes a negative P&L for a put after a big enough drop", () => {
    const e = { mode: "put", putStrike: 50, putPrem: 2, contracts: 1 };
...
describe("summarizeWeek", () => {
  it("sums realized P&L across wins and losses without netting them away individually", () => {
    const entries = [
      { realizedPnl: 300, mode: "put", putStrike: 50, contracts: 1 },
      { realizedPnl: -150, mode: "put", putStrike: 50, contracts: 1 },
    ];
  ...
  it("reports no losses as an empty array, not a zero placeholder", () => {
    const s = summarizeWeek([{ realizedPnl: 100, mode: "put", putStrike: 50, contracts: 1 }]);
  ...
  it("averages return on collateral only over entries where collateral is known", () => {
    const entries = [
      { realizedPnl: 500, mode: "put", putStrike: 50, contracts: 1 }, // collateral 5000 -> 10%
      { realizedPnl: -100, mode: "strangle", putStrike: 45, callStrike: 55, contracts: 1 }, // uncapped, excluded
    ];
    const s = summarizeWeek(entries);
  ...
  it("returns null avg return when no entry has computable collateral", () => {
    const s = summarizeWeek([{ realizedPnl: -100, mode: "strangle", putStrike: 45, callStrike: 55, contracts: 1 }]);
