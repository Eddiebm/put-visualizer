...
export function targetExpiration(targetDays = 30): string {
  const d = new Date();
...
export function computeDte(iso: string): number {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(iso)) return 30;
  return Math.max(1, Math.round((new Date(iso + "T12:00:00Z").getTime() - Date.now()) / 86400000));
}
...
export function entryDaysTo(iso: string | null | undefined): number | null {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(iso || "")) return null;
  return Math.round((new Date((iso as string) + "T12:00:00Z").getTime() - Date.now()) / 86400000);
}
