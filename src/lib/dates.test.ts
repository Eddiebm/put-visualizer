...
import { computeDte, entryDaysTo, weekStartIso, weekLabel, defaultExpiration, targetExpiration } from "./dates";
...
describe("expiration helpers", () => {
  ...
  it("targetExpiration always lands on a Friday, at least targetDays out", () => {
    ...
    const d = new Date(iso + "T12:00:00Z");
    expect(d.getUTCDay()).toBe(5);
    expect((d.getTime() - Date.now()) / 86400000).toBeGreaterThanOrEqual(29);
  });
});
