...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const snapshots: Record<string, any> = data?.snapshots || {};

    let best: { occ: string; strike: number; snap: any; dist: number } | null = null;
...
function strikeFromOcc(occ: string): number {
